...
def draw_sign_in(win, state: SignInState, blink: bool):
    ...
    center_text(win, 2, title, curses.color_pair(PAIR_MENU_TITLE) | curses.A_BOLD)
    center_text(win, 3, "Enter your credentials to continue.", curses.color_pair(PAIR_MENU_DIM))

    bw = min(50, w - 4)
    bx = (w - bw) // 2
    by = 5

    field_box_h = len(state.fields) * 4 + 3
    draw_box(win, by, bx, field_box_h, bw, PAIR_MENU_BORDER, "CREDENTIALS", PAIR_MENU_TITLE)

    for i, f in enumerate(state.fields):
        is_focus = (i == state.focused)
        fy = by + 1 + i * 4
        label_attr = (curses.color_pair(PAIR_MENU_ACCENT) | curses.A_BOLD if is_focus
                      else curses.color_pair(PAIR_MENU_DIM))
        prefix = "▶ " if is_focus else "  "
        safe_addstr(win, fy, bx+2, f"{prefix}{f['label']}", label_attr)
        input_field(win, fy+1, bx+2, bw-4, f["value"],
                    focus=is_focus, secret=f["secret"], blink=blink)

    # Buttons
    btn_y = by + field_box_h + 1
    btns = [("Enter", "[ Sign In ]"), ("Esc", "[ Back ]")]
    bxb = bx + 4
    for key, label in btns:
        safe_addstr(win, btn_y, bxb, label, curses.color_pair(PAIR_MENU_BUTTON) | curses.A_BOLD)
        bxb += len(label) + 4

    if state.message:
        msg_pair = PAIR_DANGER if "Invalid" in state.message or "failed" in state.message.lower() else PAIR_MENU_ACCENT
...
def draw_welcome_back(win, state: SignInState):
    ...
    draw_box(win, by, bx, 14, bw, PAIR_MENU_BORDER, "WELCOME BACK", PAIR_MENU_ACCENT)

    center_text(win, by+2, f"👤  {state.success_name}", curses.color_pair(PAIR_MENU_ACCENT) | curses.A_BOLD)
    center_text(win, by+4, f"Last played:  {state.success_date}", curses.color_pair(PAIR_MENU_DIM))
    center_text(win, by+5, f"Founder rank: {state.success_status}", curses.color_pair(PAIR_MENU_TITLE))
    center_text(win, by+7, "Your save data has been loaded.", curses.color_pair(PAIR_MENU_DIM))

    center_text(win, by+9, "[ Continue → ]",
                curses.color_pair(PAIR_BUTTON_FOCUS) | curses.A_BOLD)
    center_text(win, by+11, "Press Enter to continue", curses.color_pair(PAIR_MENU_DIM))
...
def draw_sign_up(win, state: SignUpState, blink: bool):
    ...
    center_text(win, 2, title, curses.color_pair(PAIR_MENU_TITLE) | curses.A_BOLD)
    center_text(win, 3, "Set up your founder profile to start building.", curses.color_pair(PAIR_MENU_DIM))
    ...
    field_box_h = len(state.fields) * 4 + 3
    draw_box(win, by, bx, field_box_h, bw, PAIR_MENU_BORDER, "ACCOUNT DETAILS", PAIR_MENU_TITLE)
    ...
    safe_addstr(win, btn_y, bx+26, "[ Back ]",
                curses.color_pair(PAIR_MENU_BUTTON) | curses.A_BOLD)

    if state.message:
        mp = PAIR_DANGER if "already" in state.message or "match" in state.message else PAIR_MENU_ACCENT
...
def draw_founder_creation(win, state: SignUpState):
    ...
    for i, bg in enumerate(BACKGROUNDS):
        ...
        name_attr = (curses.color_pair(PAIR_MENU_ACCENT) | curses.A_BOLD if is_sel
                     else curses.color_pair(PAIR_MENU_TITLE))
        safe_addstr(win, row_y, bx + 2, f"{prefix}{bg['name']}", name_attr)
        safe_addstr(win, row_y+1, bx + 6, bg["desc"][:bw-8], curses.color_pair(PAIR_MENU_DIM))
        ...
        safe_addstr(win, row_y+2, bx + 6, bstr[:bw-8],
                    curses.color_pair(PAIR_MENU_ACCENT if is_sel else PAIR_MENU_DIM))
...
def draw_ai_sub_selection(win, state: SignUpState):
    ...
    safe_addstr(win, by, bx, col_header, curses.color_pair(PAIR_MENU_TITLE) | curses.A_BOLD)
    safe_addstr(win, by+1, bx, "─"*bw, curses.color_pair(PAIR_MENU_BORDER))
    ...
    for i, sub in enumerate(AI_SUBS):
        ...
        safe_addstr(win, row_y+1, bx + 6, sub["desc"][:bw-8], curses.color_pair(PAIR_MENU_DIM))
